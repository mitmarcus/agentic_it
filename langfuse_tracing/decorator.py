"""
Decorator for tracing workflows with Langfuse.
"""
...
def trace_flow(
    ...
    """
    Decorator to add Langfuse tracing to flows.
...
def _trace_flow_class(flow_class, config, flow_name, session_id, user_id):
    ...
    def traced_run(self, shared):
        ...
        # Start trace
        self._tracer.start_trace(self._flow_name, shared)
        
        try:
            # Run the original flow
            result = original_run(self, shared) if original_run else None
            
            # End trace successfully - pass output to be recorded
            self._tracer.end_trace(output=shared, status="success")
            
            return result
            
        except Exception as e:
            # End trace with error
            self._tracer.end_trace(status="error", error=e)
            raise
        finally:
            # Ensure cleanup
            self._tracer.flush()
    ...
    def create_traced_method(self, original_method, node_id, node_name, phase):
        ...
        def traced_method(*args, **kwargs):
            self._tracer.start_node_span(node_name, node_id, phase)
            
            try:
                result = original_method(*args, **kwargs)
                self._tracer.end_node_span(input_data=args, output_data=result)
                return result
            except Exception as e:
                self._tracer.end_node_span(input_data=args, error=e)
                raise
